...
// From https://github.com/Azure/azure-iot-remote-monitoring/blob/master/Common/Helpers/AzureRetryHelper.cs
namespace Simulation.Core.Helpers
{
    ...
    public static class AzureRetryHelper
    {
        ...
        private static bool IsTransient(Exception originalException)
        {
            ...
            if (originalException is IotHubException iotHubException)
            {
                return iotHubException.IsTransient;
            }

            // If the exception is an HTTP request exception then assume it is transient
            if (originalException is HttpRequestException httpException)
            {
                return true;
            }

            if (originalException is WebException webException)
            {
